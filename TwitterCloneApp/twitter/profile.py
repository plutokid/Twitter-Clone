...
<p>'''
	...
	cur = libh.sql_query("SELECT email from `users` where `user` = '" + str(user) + "'")
	row = cur.fetchone()
	if row is not None:
		print row[0]
		print '</p><small>@'+str(user)+'</small></div>'
		if not user == libh.getUser():
			cur = libh.sql_query("SELECT follow from `follows` where `user` = '" + str(libh.getUser()) + "' and `follow` = '"+str(user)+"'")
			if cur.rowcount > 0:
	...
	cur = libh.sql_query("SELECT * from `tweets` where `user` = '" + str(user) + "' ORDER BY timestamp DESC")
